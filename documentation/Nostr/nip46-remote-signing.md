...
Server-side bunker signing now lives in `decent-newsroom/signing-bundle`. After the browser posts `/api/nostr-connect/session`, the bundle sends the session to Newsroom's `RedisRemoteSignerSessionStore` host adapter through `RemoteSignerSessionStoreInterface`. The host encrypts the NIP-46 client key and stores the remote-signer relay data in Redis; this keeps storage infrastructure outside the reusable bundle while allowing relay AUTH and future server-side publish intent to use the stored session without browser-side WebSockets.
