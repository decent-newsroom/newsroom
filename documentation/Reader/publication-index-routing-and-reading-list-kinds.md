...
- When a kind `30040` event references `30023`, `30024`, `30041`, or `30818`, the route resolves to the reading list page.
...
- `30818` (NIP-54 wiki): resolved from `event` storage and shown in the list.

For `30041` and `30818`, cards link to `/e/naddr...` so item-specific rendering remains delegated to the event route.

## Magazine category wiki support

- Magazine category pages (`/mag/{mag}/cat/{slug}`) also resolve `a`-references of kind `30818`.
...
- Kind `30818` is represented as `KindsEnum::WIKI`.
