...
- Fixed kind 0 metadata not appearing in the settings Events tab. The previous backfill attempted to read from a cache key (`pubkey_meta_`) that was almost never populated; the new implementation fetches kind 0 directly from the local strfry relay via `UserProfileService::getMetadata()` and persists it synchronously to the event table — no dependency on Messenger workers or subscription daemons.
