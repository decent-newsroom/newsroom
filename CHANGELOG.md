...
- Improved RSS feed image extraction: `media:content` URLs are now validated by `medium`, `type`, and file-extension heuristics to avoid picking up audio/video URLs as cover images. Added `media:thumbnail` and `<enclosure type="image/*">` as fallbacks. Atom feed entries now also extract cover images from `media:content` / `media:thumbnail`.
