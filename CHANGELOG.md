...
- Events received by the relay gateway are now automatically persisted to the database via Symfony Messenger: when a query correlation completes, the gateway dispatches a `PersistGatewayEventsMessage` to the async low-priority queue. The worker deduplicates by event ID (single batch query), persists new events to the Event table, and updates the graph layer (parsed_reference + current_record). This means events fetched from external relays are available locally on subsequent page loads without re-fetching.
