...
- [Bug] Fixed `cache:clear` I/O error on startup (`Failed to open stream: Input/output error` for `UnfoldBundle.php`). Root cause: Symfony's kernel boot loads every bundle class from `bundles.php` via `include()`, and the Docker bind mount (Windows → Linux) produces persistent I/O errors on the `UnfoldBundle.php` file. Fix: de-bundled both `UnfoldBundle` and `ChatBundle` — removed them from `bundles.php`, moved their DependencyInjection parameters and service scans into the main `config/services.yaml`, replaced `@UnfoldBundle`/`@ChatBundle` route notation with direct file paths, added the `@Chat` Twig namespace to `twig.yaml`, and converted `config/packages/chat.yaml` from bundle config tree to plain parameters. All code in `src/UnfoldBundle/` and `src/ChatBundle/` remains unchanged. Also made `docker-entrypoint.sh` use `--no-scripts` for initial `composer install` followed by a separate `sync` + auto-scripts pass, and added `sync` before `composer dump-autoload` in the production Dockerfile.

- [Bug] Fixed intermittent "Cannot declare class App_KernelDevDebugContainer" fatal errors and "cache directory not empty" failures caused by multiple Docker services (php, worker, worker-relay, worker-profiles, cron) racing to compile the Symfony DI container into the same shared `var/cache/` directory. Asset compilation, database migrations, and cache warmup now run only in the primary FrankenPHP container; all worker/cron services wait for `service_healthy` before starting. Removed a stale `src/var/cache/` directory that was created by a `getProjectDir()` mis-resolution.
