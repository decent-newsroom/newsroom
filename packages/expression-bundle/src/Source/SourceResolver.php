...
final class SourceResolver implements SourceResolverInterface
{
    public function __construct(
        ...
        private readonly Bech32EncoderInterface $bech32Encoder,
    ) {}
    ...
    private function decodeBech32Input(array $inputRef): array
    {
        ...
        if (str_starts_with($ref, 'nevent1') || str_starts_with($ref, 'note1')) {
            try {
                $decoded = $this->bech32Encoder->decodeComplexEntity($ref);
        ...
        if (str_starts_with($ref, 'naddr1')) {
            try {
                ...
                if (isset($decoded['kind'], $decoded['pubkey'], $decoded['identifier'])) {
                    $address = $decoded['kind'] . ':' . $decoded['pubkey'] . ':' . $decoded['identifier'];
                    return ['a', $address];
                }
            } catch (\Throwable $e) {
